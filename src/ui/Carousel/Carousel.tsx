...
export function Carousel({
  items, orientation = "horizontal", slidesPerView = 1, align = "start", loop = false,
  ...
  // Pages come from where each slide sits, so they follow slidesPerView, align and the carousel's width.
  const measure = useCallback(() => {
    ...
    for (const el of Array.from(track.children) as HTMLElement[]) {
      ...
      const target = Math.round(Math.min(Math.max(align === "center" ? start - (size - length) / 2 : start, 0), max));
  ...
  const control = (dir: "prev" | "next") => {
    ...
    return (
      <span className={styles.control}>
        <Tooltip content={name} placement={vertical ? (dir === "prev" ? "top" : "bottom") : dir === "prev" ? "left" : "right"}>
  ...
  return (
    <div
      className={[styles.carousel, styles[orientation], PER[slidesPerView], align === "center" ? styles.center : ""].join(" ")}
