...
export type LinkIntent = "brand" | "neutral";
...
export function Link({ children, href, onClick, intent = "brand", external = false, disabled = false }: LinkProps) {
  const cls = [styles.link, styles[intent], disabled ? styles.disabled : ""].join(" ");
  const body = (
    <>
      {children}
      {external && (
